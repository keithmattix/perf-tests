...
# Create a waypoint for the namespace
istioctl waypoint apply --enroll-namespace --wait 120s

WAYPOINT_POD_NAME=$(kubectl get po -listio.io/gateway-name=waypoint -o jsonpath="{.items[*]['metadata.name']}")

istioctl pc log "$WAYPOINT_POD_NAME" --level debug

# Now create an L7 policy to allow all HTTP requests to port 8080
kubectl apply -f - <<EOF
...
# Optionally, in another tab, watch the resource usage of the waypoint Envoy:
watch kubectl top pod "$WAYPOINT_POD_NAME" --containers
